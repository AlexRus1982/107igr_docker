{{-- main_url --}}
@php
    $list = json_decode(json_encode($page->list, JSON_UNESCAPED_UNICODE));
    // logger(json_encode(array_keys((array)$list), JSON_UNESCAPED_UNICODE));
    // logger(json_encode($list, JSON_UNESCAPED_UNICODE));

    $currentPage = (int) $list->current_page;
    $title = ($currentPage > 1) ? "{$page->category->category} - страница №{$currentPage}" : $page->category->category;
@endphp
...
@section('main-section')
    <div class="bread-crumps-wrapper">
        <div class="bread-crumps">
            ...
            <div class="category_link">{{ $page->category->category }}</div>
        </div>
    </div>
    ...
    {{ $page->list->links('main.includes.pagination') }}

    @if ($page->category->description != '')
        <div class="category-page-description-container">
            <div class="category-page-description-wrapper">
