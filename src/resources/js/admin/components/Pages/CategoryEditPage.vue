    ...
    <div class="apps-page">
        ...
        <div class="apps-body">
            ...
            <div class="first-block d-flex">
                ...
                <div class="text-section">
                    <div class="app-section d-flex">
                        <div class="app-section-name-label label me-2">Название категории</div>
                        <input class="app-section-name-input" type="text" v-model="appCategoryData.category">
                    </div>
        
                    <div class="app-section d-flex">
                        <div class="app-section-name-label label me-2">Синоним URL</div>
                        <input class="app-section-name-input" type="text" v-model="appCategoryData.url">
                ...
                <div class="image-section">
                    <IconImage :imagePath = "appCategoryData.image" @imageChanged="onIconChanged"></IconImage>
                </div>
            </div>

            <div class="me-auto form-check form-switch form-check-reverse" style="font-size: 1.1rem;">
                <label class="form-check-label" for="activityCheck" style="margin-right: 109px;">Активность</label>
                <input 
                    class="ms-auto form-check-input"
                    id="activityCheck"
                    type="checkbox"
                    role="switch"
                    :checked="appCategoryData.activity == 1 ? true : false"
            ...
            <div class="parent-wrapper">
                <div class="parent-label">Родитель</div>
                <div class="parent-text">{{ appCategoryData.mainCategory }}</div>
            </div>

            <div class="order-wrapper">
                <div class="order-label">Номер по порядку</div>
                <div class="order-text">{{ appCategoryData.order }}</div>
            </div>
            ...
            <div class="text-editor-label" style="font-size: 1.5rem;">Статистика</div>
            <div class="stat-active-apps">
                <span>Приложений в категории </span>
                <span>{{ appCategoryStat.category_active_count }}</span> / <span>{{ appCategoryStat.category_count }}</span>
            </div>

            <div class="stat-icon">
                <img :src="appCategoryStat.icon">
                <div class="stat-icon-label">Иконка самого скачиваемого приложения</div>
            </div>
        </div>
...
<script lang="ts">
    ...
    export default {
        ...
        setup() {
            ...
            const appCategoryData = ref(<any>[]);
            const appCategoryStat = ref(<any>[]);

            const appMessageWarn = ref('');
            const appMessageSuccess = ref('');

            const DTO_description = ref(appCategoryData.description);

            console.debug(url, category_id);

            fetch(`/../api/category/info`, {
                    method: 'POST',
                    headers: {
            ...
            .then(
                ({
                    ...
                    console.debug(JSON.stringify(Object.keys(categoryInfo)));
                    console.debug(categoryInfo);
                    
                    // app.icon = '/img/tmp.png';
                    // app.icon = '';

                    appCategoryData.value = categoryInfo.category;
                    appCategoryStat.value = categoryInfo;
                    DTO_description.value = categoryInfo.category.description;
        ...
        methods: {
            onIconChanged(iconPath) {
                this.appCategoryData.image = iconPath;
            },

            onTextChanged(text) {
                this.appCategoryData.description = text;
            },

            saveValues() {
                console.debug(this.appCategoryData);
                return;
            ...
            viewApp(event) {
                const appUrl = `/../${this.appCategoryData.main_url}/${this.appCategoryData.url}`;
            ...
            makeTranslit() {
                this.appCategoryData.url = this.appCategoryData.name.translit();
            },

            switchActivity() {
                this.appCategoryData.activity = (this.appCategoryData.activity == '0') ? '1' : '0';
            },
        },
    }
</script>